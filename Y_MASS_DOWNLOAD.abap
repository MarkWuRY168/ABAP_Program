PROGRAM y_mass_download.
" ---------------------------------------------------------------------------------------
" SAP Tables
" ---------------------------------------------------------------------------------------
TABLES: trdir,seoclass,tfdir,enlfdir,dd02l,tadiv,dd40l,transfdesc,badi_spot,sxc_exit.
...
CONSTANTS ss_code           TYPE c LENGTH 1 VALUE 'C'.
CONSTANTS ss_table          TYPE c LENGTH 1 VALUE 'T'.

" ---------------------------------------------------------------------------------------
" Global variables
" ---------------------------------------------------------------------------------------
DATA statusbarmessage       TYPE c LENGTH 100.
...
DATA gv_merge_flag     TYPE char1. " added by lijq
DATA gv_functiongroup  LIKE enlfdir-area.
DATA gv_different_flag TYPE char1.
" ---------------------------------------------------------------------------------------
...
SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE tblock2.
  ...
  SELECTION-SCREEN BEGIN OF LINE.
    ...
    SELECTION-SCREEN COMMENT 5(16) trfunc.
    SELECTION-SCREEN COMMENT 30(25) tfincld.
    PARAMETERS pincld AS CHECKBOX.
  SELECTION-SCREEN END OF LINE.
  ...
  "  Enhancement Spot
  SELECTION-SCREEN BEGIN OF LINE.
    PARAMETERS rspot RADIOBUTTON GROUP r1.
    SELECTION-SCREEN COMMENT 5(30) trspot.
  SELECTION-SCREEN END OF LINE.

  SELECTION-SCREEN BEGIN OF LINE.
    SELECTION-SCREEN COMMENT 10(15) tpsname.
    SELECT-OPTIONS sospot FOR badi_spot-enhspotname.
  SELECTION-SCREEN END OF LINE.
  " BADI Name
  SELECTION-SCREEN BEGIN OF LINE.
    PARAMETERS rbadi RADIOBUTTON GROUP r1.
    SELECTION-SCREEN COMMENT 5(30) trbadi.
  SELECTION-SCREEN END OF LINE.

  SELECTION-SCREEN BEGIN OF LINE.
    SELECTION-SCREEN COMMENT 10(15) tpbname.
    SELECT-OPTIONS sobadi FOR sxc_exit-exit_name.
  SELECTION-SCREEN END OF LINE.
  " XSLT
  SELECTION-SCREEN BEGIN OF LINE.
...
SELECTION-SCREEN BEGIN OF BLOCK b4 WITH FRAME TITLE tblock4.
  SELECTION-SCREEN BEGIN OF LINE.
    ...
    PARAMETERS phtml RADIOBUTTON GROUP g1 DEFAULT 'X' USER-COMMAND a.
  SELECTION-SCREEN END OF LINE.
  ...
  SELECTION-SCREEN BEGIN OF LINE.
    ...
    PARAMETERS pfolder LIKE rlgrap-filename MEMORY ID mfolder DEFAULT 'D:\test1'.
  SELECTION-SCREEN END OF LINE.
...
AT SELECTION-SCREEN ON plogical.
  " ---------------------------------------------------------------------------------------
  IF pserv IS NOT INITIAL.
    CALL FUNCTION 'FILE_GET_NAME'
      EXPORTING  logical_filename = plogical
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR soxslt-low.
  "  ---------------------------------------------------------------------------------------------------------------------
  CALL FUNCTION 'REPOSITORY_INFO_SYSTEM_F4'
    EXPORTING  object_type           = 'XSLT'
               object_name           = soxslt-low
               suppress_selection    = 'X'
               use_alv_grid          = ''
               without_personal_list = ''
    IMPORTING  object_name_selected  = soxslt-low
    EXCEPTIONS cancel                = 1.
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR soxslt-high.
  ...
  CALL FUNCTION 'REPOSITORY_INFO_SYSTEM_F4'
    EXPORTING  object_type           = 'XSLT'
               ...
               use_alv_grid          = ''
               without_personal_list = ''
    IMPORTING  object_name_selected  = soxslt-high
    EXCEPTIONS cancel                = 1.
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR soclass-high.
  ...
  "  ---------------------------------------------------------------------------------------------------------------------
AT SELECTION-SCREEN ON VALUE-REQUEST FOR sospot-low.
  "  ---------------------------------------------------------------------------------------------------------------------
  CALL FUNCTION 'REPOSITORY_INFO_SYSTEM_F4'
    EXPORTING  object_type           = 'XS'
               object_name           = sospot-low
               suppress_selection    = 'X'
               variant               = 'SAP&BADI'
               use_alv_grid          = ''
               without_personal_list = ''
    IMPORTING  object_name_selected  = sospot-low
    EXCEPTIONS cancel                = 1.

  "  ---------------------------------------------------------------------------------------------------------------------
AT SELECTION-SCREEN ON VALUE-REQUEST FOR sospot-high.
  "  ---------------------------------------------------------------------------------------------------------------------
  CALL FUNCTION 'REPOSITORY_INFO_SYSTEM_F4'
    EXPORTING  object_type           = 'XS'
               object_name           = sospot-high
               suppress_selection    = 'X'
               variant               = 'SAP&BADI'
               use_alv_grid          = ''
               without_personal_list = ''
    IMPORTING  object_name_selected  = sospot-high
    EXCEPTIONS cancel                = 1.
...
INITIALIZATION.
  ...
  tfincld = 'Remove FM''s floder in FG: '.
  trclass = 'Classes'.
  tpcname = 'Class name'.
  tmess   = 'Message class'.
  tmname  = 'Class name'.
  trspot  = 'Enhancement Spot'.
  tpsname = 'Enh Spot name'.
  trbadi  = 'BADI'.
  tpbname = 'BADI Name'.
  tmlang  = 'Language'.
  tprog   = 'Programs'.
...
START-OF-SELECTION.
  ...
  CASE 'X'.
    ...
    " ---added by lijq ---begin----
    "    Select Classes / Enhancement Spot / * BADI Name
    "     WHEN rclass.
    WHEN rclass OR rspot OR rbadi.
      " ---added by lijq ---end----
      PERFORM retrieveclasses
        USING iclasses[] ifunctions[] soclassname[] " Class name
              soauthor[] " Author
              customernamespace " Customer name range
              ...
              pfunc " Get functions
              pinc " Get includes
              ptrans precf " Search recursively for functions
    ...
    WHEN rprog.
      PERFORM retrieveprograms
        USING iprograms[] iprogfunctions[] soprogramname[] " Program name
              soauthor[] " Author
              customernamespace " Customer name range
              pmod " Also modified by author
              pcust " Customer object only
    ...
    WHEN rxslt.
      PERFORM retrievexslt
        USING itransformations[]
    ...
    CASE 'X'.
      ...
      WHEN rmess.
        IF NOT ( imessages[] IS INITIAL ).
          SORT imessages ASCENDING BY arbgb
                                      msgnr.
          LOOP AT imessages.
            APPEND imessages TO isinglemessageclass.
            AT END OF arbgb.
              PERFORM downloadmessageclass
                USING isinglemessageclass[]
                      imessages-arbgb
                      downloadfolder
      ...
      WHEN rfunc.
        IF NOT ( ifunctions[] IS INITIAL ).
          PERFORM downloadfunctions
            USING ifunctions[] idictfilename[] itabletypefilename[] downloadfolder downloadfileextension space pfdoc
                  phtml customernamespace pinc pdict textextension htmlextension psortt slashseparatortouse pserv
                  ppromess serverfilesystem pback.
        ENDIF.

      " ---added by lijq ---begin----
      "    Download Classes / Enhancement Spot / * BADI Name
      "     WHEN rclass.
      WHEN rclass OR rspot OR rbadi.
        " ---added by lijq ---end----
        IF NOT ( iclasses[] IS INITIAL ).
          PERFORM downloadclasses
            USING iclasses[] ifunctions[] idictfilename[] itabletypefilename[] downloadfolder downloadfileextension
                  htmlextension textextension phtml customernamespace pinc pdict pcdoc psortt slashseparatortouse pserv
                  ppromess serverfilesystem pback.
        ENDIF.
      ...
      WHEN rprog.
        IF NOT ( iprograms[] IS INITIAL ).
          PERFORM downloadprograms
            USING iprograms[] iprogfunctions[] idictfilename[] itabletypefilename[] downloadfolder downloadfileextension
                  htmlextension textextension phtml customernamespace pinc pdict '' " Documentation
                  psortt slashseparatortouse pserv ppromess serverfilesystem pback.
        ENDIF.

      WHEN rxslt.
        IF NOT ( itransformations[] IS INITIAL ).
          PERFORM downloadxslt
            USING itransformations[]
                  downloadfolder
                  downloadfileextension
                  htmlextension
...
*    WHEN rclass.
      ...
      ELSE.
        ...
        PERFORM displaystatus USING statusbarmessage
                                    2.
      ENDIF.
    ENDIF.
  ENDIF.
  ...
  " ---------------------------------------------------------------------------------------
FORM freememory
  USING ilocprograms       LIKE iprograms[]
...
FORM checkcomboboxes.
  ...
  IF statusbarmessage IS NOT INITIAL.
    PERFORM displaystatus USING statusbarmessage
                                3.
    forcedexit = 1.
    STOP.
...
FORM fillselectionranges.
  ...
  "  Enhancement Spot
  CLEAR it_spot.
  IF sospot[] IS NOT INITIAL.

    SELECT a~spotname
           b~enhname
           b~class_name
      INTO CORRESPONDING FIELDS OF TABLE it_spot
      FROM badiimpl_enh AS a
             INNER JOIN
               badi_impl AS b ON a~enhname = b~enhname
      WHERE a~spotname IN sospot[]
        AND a~active = 'X'
        AND ( b~class_name LIKE 'Z%'
           OR b~class_name LIKE 'Y%' ).
    IF it_spot IS NOT INITIAL.
      SORT it_spot BY class_name.
      DELETE ADJACENT DUPLICATES FROM it_spot COMPARING class_name.

      CLEAR soclass.
      soclass-sign   = 'I'.
      soclass-option = 'EQ'.
      LOOP AT it_spot INTO is_spot.
        soclass-low = is_spot-class_name.
        APPEND soclass.
      ENDLOOP.
    ENDIF.
  ...
  " BADI Name
  CLEAR it_badi.
  IF sobadi[] IS NOT INITIAL.
    SELECT imp_class exit_name FROM v_ext_imp
      INTO TABLE it_badi
      WHERE active = 'X'
        AND exit_name IN sobadi[]
        AND ( imp_class LIKE 'Z%'
           OR imp_class LIKE 'Y%' ).

    IF it_badi IS NOT INITIAL.
      SORT it_badi BY imp_class.
      DELETE ADJACENT DUPLICATES FROM it_badi COMPARING imp_class.

      CLEAR soclass.
      soclass-sign   = 'I'.
      soclass-option = 'EQ'.
      LOOP AT it_badi INTO is_badi.
        soclass-low = is_badi-imp_class.
        APPEND soclass.
      ENDLOOP.
    ENDIF.
...
" ---------------------------------------------------------------------------------------
FORM retrievetables
  USING ilocdictstructure   LIKE idictionary[]
  ...
  SELECT a~tabname AS tablename INTO CORRESPONDING FIELDS OF TABLE idictstructure
    FROM dd02l AS a
           INNER JOIN
             tadir AS b ON a~tabname = b~obj_name
...
" retrieveTableTypes
" ---------------------------------------------------------------------------------------
FORM retrievetabletypes
  USING iloctabletypes      LIKE itabletypes[]
        sotabletypenames    LIKE sotable[]
        soauthor            LIKE soauthor[]
        VALUE(solocpackage) LIKE sopack[].

  SELECT * INTO CORRESPONDING FIELDS OF TABLE iloctabletypes
    FROM dd40l AS a
           INNER JOIN
             dd40t AS t ON a~typename = t~typename
               INNER JOIN
                 tadir AS b ON a~typename = b~obj_name
...
" ------------------------------------------------------------------------
FORM retrievexslt
  USING iloctransformations         LIKE itransformations[]
  ...
  IF rangexslt[] IS INITIAL.
    ...
    PERFORM findallxsltforauthor
      USING iloctransformations[]
            rangexslt[]
            rangeauthor[]
            custnamerange
...
FORM findtabledefinition USING VALUE(tablename)
  ...
  CALL FUNCTION 'DD_INT_TABL_GET'
    EXPORTING  tabname        = tablename
               langu          = pmlang
    IMPORTING  gotstate       = gotstate
               dd02v_n        = wadd02v_n
    TABLES     dd03p_n        = definition
    EXCEPTIONS internal_error = 1.

  IF sy-subrc = 0 AND wadd02v_n IS NOT INITIAL.
    CALL FUNCTION 'DD_TABL_EXPAND'
      EXPORTING  dd02v_wa          = wadd02v_n
                 mode              = 46
                 prid              = 0
      TABLES     dd03p_tab         = definition
  ...
  IF sy-subrc = 0 AND gotstate = 'A'.
    LOOP AT definition.
      ...
      CALL FUNCTION 'DD_DOMA_GET'
        EXPORTING  domain_name   = definition-domname
                   get_state     = 'M  '
                   langu         = pmlang
*                   prid          = 0
                   withtext      = 'X'
        TABLES     dd07v_tab_a   = idomaindataa
...
" ---------------------------------------------------------------------------------------
FORM retrievemessageclass
  USING ilocmessages            LIKE imessages[]
        rangeauthor             LIKE soauthor[]
  ...
  IF messageclassname IS NOT INITIAL.
    " Check to see if the message class exists in the package
    IF solocpackage[] IS NOT INITIAL.
      SELECT obj_name AS arbgb INTO TABLE imclasses
        FROM tadir
        WHERE pgmid     = 'R3TR'
          AND object    = 'MSAG'
    ...
    IF imclasses[] IS INITIAL.
      SELECT t100~arbgb
        ...
        FROM t100
               INNER JOIN
                 t100a ON t100a~arbgb = t100~arbgb
      ...
      SELECT t100~arbgb                                "#EC CI_BUFFJOIN
        ...
        FROM t100
               INNER JOIN
                 t100a ON t100a~arbgb = t100~arbgb
                   INNER JOIN
                     tadir ON t100~arbgb = tadir~obj_name
        WHERE t100a~masterlang  = messageclasslang
          AND t100a~respuser   IN rangeauthor[]
...
" ---------------------------------------------------------------------------------------
FORM retrievefunctions
  USING sofname                  LIKE sofunctionname[]
  ...
  SELECT a~funcname AS functionname
    ...
    FROM v_fdir AS a
           INNER JOIN
             tlibv AS b ON a~area = b~area
               INNER JOIN
                 tadir AS c ON a~area = c~obj_name
    WHERE a~funcname IN sofname[]
      AND a~area     IN sofgroup[]
*           AND a~generated = ''           "    LIJQ
  ...
  LOOP AT ilocfunctionnames INTO wafunctionname.
    ...
    PERFORM scanforfunctionincludes
      USING wafunctionname-progname
            customeronly
            customernamerange
    ...
    IF gettextelements IS NOT INITIAL.
...
" ---------------------------------------------------------------------------------------
FORM retrievefunctiondetail
  USING VALUE(functionname)
        progname
        includename
        titletext.
...
" ---------------------------------------------------------------------------------------
FORM scanforadditionalfuncstuff
  USING ilocfunctions                   LIKE ifunctions[]
  ...
  LOOP AT ilocfunctions INTO wafunction.
    IF searchforincludes IS NOT INITIAL.
      ...
      PERFORM scanforincludeprograms
        USING wafunction-topincludename
              recursiveincludes
              customeronly
              customernamerange
              wafunction-iincludes[].
    ...
    IF searchforfunctions IS NOT INITIAL.
  ...
  LOOP AT ilocfunctions INTO wafunction.
    " Now we have everything perhaps we had better find all the dictionary structures
    IF searchfordictionary IS NOT INITIAL.
      PERFORM scanfortables
        USING wafunction-progname
              customeronly
              customernamerange
              wafunction-idictstruct[].

      PERFORM scanforlikeortype
        USING wafunction-progname
              customeronly
              customernamerange
              wafunction-idictstruct[]
    ...
    IF searchformessages IS NOT INITIAL.
    ...
    IF searchfortransformations IS NOT INITIAL.
      PERFORM scanfortransformations
        USING wafunction-progname
              customeronly
              customernamerange
...
FORM scanforclasses
  ...
  " Build the name of the possible customer exception classes
...
" ---------------------------------------------------------------------------------------
FORM scanforincludeprograms
  USING VALUE(programname)
        VALUE(recursiveincludes)
        VALUE(customeronly)
        VALUE(customernamerange)
  ...
  LOOP AT itokens WHERE str = include AND type = 'I'.
    ...
    IF customeronly IS NOT INITIAL.
      TRY.
    ...
    IF sy-subrc <> 0.
      ...
      IF recursiveincludes IS NOT INITIAL.
...
FORM scanfortransformations
  ...
  LOOP AT itokens WHERE str = transformation AND type = 'I'.
    ...
    IF sy-subrc <> 0.
      ...
      PERFORM scanfortransincludes
        USING watransformation-xsltname
              customeronly
              customernamerange
              iloctransformation[].
...
" ---------------------------------------------------------------------------------------
FORM scanfortransincludes
  USING VALUE(xsltname)
        VALUE(customeronly)
        VALUE(customernamerange)
  ...
  LOOP AT ixsltsource INTO waxsltsource.
    IF waxsltsource-line NS '<xsl:include'.
      CONTINUE.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ...
      IF watokens-str <> destination.
        ...
        IF sy-subrc = 0.
          ...
          IF recursivefunctions IS NOT INITIAL.
            PERFORM scanforfunctions
              USING wafunction-functionmaininclude
                    space
                    recursiveincludes
...
FORM scanforfunctionincludes
  ...
  LOOP AT itokens WHERE str = include AND type = 'I'.
    ...
    READ TABLE itokens INDEX nextline INTO watokens.
*      IF watokens-str CP '*F++'.     "LIJQ
    IF NOT ( ( watokens-str CP '*F++' OR watokens-str+0(2) = 'LY' OR watokens-str+0(2) = 'LZ' ) AND watokens-str NP '*U++' ). " LIJQ
      CONTINUE.
    ENDIF.
...
" ---------------------------------------------------------------------------------------
FORM retrieveprograms
  USING ilocprogram                   LIKE iprograms[]
        ilocfunctions                 LIKE ifunctions[]
  ...
  IF rangeprogram[] IS INITIAL.
    " We are finding all programs by an author
    PERFORM findallprogramsforauthor
      USING ilocprogram[]
            rangeprogram[]
            rangeauthor[]
            custnamerange
  ...
  ELSE.
    ...
    IF warangeprogram-low CS asterix.
      PERFORM findprogramsbywildcard
        USING ilocprogram[]
              rangeprogram[]
              rangeauthor[]
              custnamerange
  ...
  " Find extra items
  PERFORM scanforadditionalprogstuff
    USING ilocprogram[]
          ilocfunctions[]
          gettextelements
          getmessages
...
" ------------------------------------------------------------------------
FORM scanforadditionalprogstuff
  USING ilocprogram                   LIKE iprograms[]
        ilocfunctions                 LIKE ifunctions[]
        VALUE(gettextelements)
        VALUE(getmessages)
  ...
  IF getincludes IS NOT INITIAL.
    LOOP AT ilocprogram INTO waprogram.
      mytabix = sy-tabix.
      PERFORM scanforincludeprograms
        USING waprogram-progname
              recursiveincludesearch
              customeronly
              customernamerange
              waprogram-iincludes[].
  ...
  IF getfunctions IS NOT INITIAL.
    LOOP AT ilocprogram INTO waprogram.
...
FORM findprogramdetails
  ...
  " Search for any GUI texts
  IF getscreens IS NOT INITIAL AND ( programtype = 'M' OR programtype = '1' ).
    PERFORM retrieveguititles USING waprogram-iguititle[]
                                    programname.
  ENDIF.

  " Find individual messages
  IF getmessages IS NOT INITIAL.
  ...
  IF gettransformations IS NOT INITIAL.
    PERFORM scanfortransformations
      USING programname
            customeronly
            customernamerange
  ...
  IF getcustdictstructures IS NOT INITIAL.
    PERFORM scanfortables
      USING programname
            customeronly
            customernamerange
...
FORM findallprogramsforauthor
  ...
  IF alsomodifiedbyauthor IS NOT INITIAL.
    ...
    IF customerprogsonly IS INITIAL.
      ...
      SELECT a~progname
        ...
        FROM reposrc AS a
               INNER JOIN
                 tadir AS b ON a~progname = b~obj_name
...
FORM findallxsltforauthor
  ...
  IF alsomodifiedbyauthor IS NOT INITIAL.
    ...
    IF customerprogsonly IS INITIAL.
      ...
      SELECT obj_name INTO watransformation-xsltname
        FROM tadir
        WHERE pgmid     = 'R3TR'
          AND object    = 'XSLT'
...
FORM checkxsltdoesexist USING iloctransformations LIKE itransformations[]
  ...
  SELECT SINGLE obj_name INTO watransformation-xsltname
    FROM tadir
    WHERE pgmid     = 'R3TR'
      AND object    = 'XSLT'
      AND obj_name IN rangexslt.
...
FORM retrieveprogramtexts USING ilocselectiontexts LIKE dumitexttab[]
  ...
  DATA castprogramname TYPE c LENGTH 50.
  ...
  LOOP AT itexttable WHERE id = 'S'.
    watexts-key   = itexttable-key.
    watexts-entry = itexttable-entry.
...
FORM findmainmessageclass USING VALUE(programname)
  ...
  SELECT SINGLE Msgid FROM trdire
    INTO MessageClass
...
" ------------------------------------------------------------------------
FORM retrieveclasses
  USING ilocclasses                   LIKE iclasses[]
        ilocfunctions                 LIKE ifunctions[]
  ...
  IF rangeclass[] IS INITIAL.
    " We are finding all programs by an author
    PERFORM findallclassesforauthor
      USING ilocclasses[]
            rangeclass[]
            rangeauthor[]
            custnamerange
  ...
  " Find extra items
  IF ilocclasses[] IS NOT INITIAL.
    PERFORM scanforadditionalclassstuff
      USING ilocclasses[]
            ilocfunctions[]
            gettextelements
            getmessages
...
" ------------------------------------------------------------------------
FORM findallclassesforauthor
  USING ilocclass                   LIKE iclasses[]
        rangeclass                  LIKE soclassname[]
        rangeauthor                 LIKE soauthor[]
        VALUE(custnamerange)
        VALUE(alsomodifiedbyauthor)
  ...
  IF alsomodifiedbyauthor IS NOT INITIAL.
    ...
    IF customerclassesonly IS INITIAL.
      " Select all classes
      SELECT clsname descript msg_id FROM vseoclass
...
*               AND ( clsname LIKE 'ZC%' OR clsname LIKE 'YC%' )
          AND ( clsname LIKE 'Z%' OR clsname LIKE 'Y%' )    " LIJQ
          AND ( author IN rangeauthor OR changedby IN rangeauthor )
          AND version = '1'
          AND ( state = '0' OR state = '1' ).
      IF sy-subrc <> 0.
...
FORM checkclassdoesexist USING ilocclass       LIKE iclasses[]
  ...
  DATA waclass TYPE tclass.
  " ---added by lijq ---begin----
  DATA wtclass TYPE TABLE OF tclass.

  SELECT clsname descript msg_id FROM vseoclass
    INTO CORRESPONDING FIELDS OF TABLE wtclass
    WHERE clsname IN rangeclass
      AND version  = '1'
      AND langu    = language
      AND ( state = '0' OR state = '1' ).
  ...
  LOOP AT wtclass INTO waclass.
    IF waclass-clsname IS NOT INITIAL.
...
" ------------------------------------------------------------------------
FORM scanforadditionalclassstuff
  USING ilocclasses                   LIKE iclasses[]
        ilocfunctions                 LIKE ifunctions[]
        VALUE(gettextelements)
        VALUE(getmessages)
  ...
  LOOP AT ilocclasses INTO waclass WHERE scanned IS INITIAL.
    ...
    mytabix = sy-tabix.
    PERFORM findclassdetails
      USING waclass-clsname
            waclass
            ilocfunctions[]
            gettextelements
            getmessages
  ...
  IF recursiveclasssearch IS INITIAL.
    RETURN.
  ENDIF.
  ...
  LOOP AT ilocclasses INTO waclass.
    ...
    IF waclass-clsname CS 'ZCX_' AND waclass-clsname CS 'CX_'.
      CONTINUE.
    ENDIF.

    " Find any classes defined in the main class definition
    PERFORM scanforclasses
      USING waclass-privateclasskey
            waclass-clsname
            customeronly
            customernamerange
            ilocclasses[]
    ...
    PERFORM scanforclasses
      USING waclass-protectedclasskey
            waclass-clsname
            customeronly
            customernamerange
            ilocclasses[]
...
FORM findclassdetails
  ...
  ELSE.
    ...
    IF waclass-msg_id IS NOT INITIAL AND waclass-imessages[] IS INITIAL.
      PERFORM retrievemessageclass
        USING waclass-imessages[]
              rnblankauthor[]
              waclass-msg_id
              pmlang
              ''
...
FORM findinterfacemethods USING VALUE(classname)
  ...
  LOOP AT ilocinterfaces INTO wainterface.
    SELECT a~clsname  AS interfacename
      ...
      FROM seocompo AS a
             INNER JOIN
               seocompodf AS b ON  a~clsname = b~clsname
  ...
  LOOP AT iintmethod.
    ...
    SELECT SINGLE descript FROM seocompotx
    ...
    IF wamethod-methodkey IS NOT INITIAL.
...
FORM findclassparent USING VALUE(classname)
  ...
  CALL FUNCTION 'SEO_INHERITANC_READ'
    EXPORTING  clskey             = clskey
               version            = seoc_version_active
    IMPORTING  inheritance        = wainheritance
...
FORM buildclasskeys USING waclass TYPE tclass.
  ...
  waclass-publicclasskey    = cl_oo_classname_service=>get_pubsec_name( clsname = waclass-clsname ).

  waclass-privateclasskey   = cl_oo_classname_service=>get_prisec_name( clsname = waclass-clsname ).

  waclass-protectedclasskey = cl_oo_classname_service=>get_prosec_name( clsname = waclass-clsname ).
...
FORM scanformessages USING VALUE(programname)
  ...
  LOOP AT itokens.
    ...
    IF wamessage-arbgb IS NOT INITIAL AND wamessage-msgnr IS NOT INITIAL AND wamessage-text IS INITIAL.
    ...
    " Append the message
    IF wamessage IS NOT INITIAL.
      IF wamessage-text IS NOT INITIAL.
...
FORM findprogramscreenflow USING waprogram TYPE tprogram.
  ...
  CALL FUNCTION 'DYNPRO_PROCESSINGLOGIC'
    EXPORTING rep_name  = waprogram-progname
    TABLES    scr_logic = iflow.
...
FORM findmainfunctioninclude
  ...
  DATA iresults         TYPE match_result_tab.
  DATA waresult         TYPE match_result.
  DATA startingposition TYPE i.
...
FORM findfunctionscreenflow USING wafunction TYPE tfunction.
  ...
  CALL FUNCTION 'DYNPRO_PROCESSINGLOGIC'
    EXPORTING rep_name  = wafunction-progname
    TABLES    scr_logic = iflow.
...
FORM removeleadingzeros CHANGING myvalue.
  CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
    EXPORTING  input  = myvalue
    IMPORTING  output = myvalue
    EXCEPTIONS OTHERS = 1.
ENDFORM.
...
FORM determinefrontendopsystem USING separator
  ...
  objfile->get_platform( RECEIVING  platform             = platformid
                         EXCEPTIONS cntl_error           = 1
                                    error_no_gui         = 2
                                    not_supported_by_gui = 3 ).
  CASE platformid.
...
FORM determineserveropsystem USING separator
  ...
  SELECT SINGLE filesys FROM opsystem
...
FORM findexternalcommand USING VALUE(locserverfilesystem).
  ...
  CALL FUNCTION 'SXPG_COMMAND_CHECK'
    EXPORTING  commandname       = 'ZDTX_MKDIR'
               operatingsystem   = castserveropsys
    EXCEPTIONS command_not_found = 1
               OTHERS            = 0.
...
FORM downloadddstructures
  USING ilocdictionary                LIKE idictionary[]
  ...
  LOOP AT ilocdictionary ASSIGNING <wadictionary>.
    ...
    IF sy-subrc <> 0.
      ...
      IF sy-subrc <> 0.
        ...
        PERFORM convertddtohtml
          USING <wadictionary>-istructure[]
                ihtmltable[]
                <wadictionary>-tablename
                <wadictionary>-tabletitle
                sorttablesasc
  ...
  LOOP AT iloctabletypes ASSIGNING <watabletypes>.
    ...
    IF sy-subrc <> 0.
      ...
      IF sy-subrc <> 0.
        ...
        PERFORM converttabletypetohtml
          USING iloctabletypes[]
                ihtmltable[]
                <watabletypes>-typename
                <watabletypes>-ddtext
                sorttablesasc
...
FORM downloadfunctions
  ...
  " ---added by lijq-----being-----
  " --remove the DUPLICATES include files from the same FG when download FG
  IF pincld = 'X' AND sofgroup[] IS NOT INITIAL.

    DATA lv_functiongroup LIKE enlfdir-area.
    CLEAR lv_functiongroup.

    SORT ilocfunctions BY functiongroup
                          includenumber.

    LOOP AT ilocfunctions ASSIGNING <wafunction>.
      IF <wafunction>-functiongroup NOT IN sofgroup[].
        CONTINUE.
      ENDIF.
      IF lv_functiongroup = <wafunction>-functiongroup.
        CLEAR <wafunction>-iincludes.
      ELSE.
        lv_functiongroup = <wafunction>-functiongroup.
      ENDIF.
    ENDLOOP.
  ENDIF.
  " ---added by lijq-----end-----
...
*---added by lijq-----being----*
*---chenge code sequence
*---from
**" 1 perform downloadFunctionDocs
**" 2 perform readFunctionAndDownload
**" 3 perform readIncludeAndDownload
**" 4 perform downloadScreens
*----to-----
**" 2 PERFORM readfunctionanddownload
**" 3 PERFORM readincludeanddownload
**" 1 PERFORM downloadfunctiondocs
**" 4 PERFORM downloadscreens
*---create FG floder.
    DATA sub_floder_name TYPE string.
    ...
    IF subdir IS INITIAL.
      ...
      IF rfunc = 'X' AND sofgroup[] IS NOT INITIAL AND <wafunction>-functiongroup IN sofgroup[].
        CONCATENATE 'FG_' <wafunction>-functiongroup INTO sub_floder_name.
        IF pincld = 'X'.
          gv_merge_flag = 'X'.

          IF gv_functiongroup <> sub_floder_name.
            gv_different_flag = 'X'.
          ENDIF.
          gv_functiongroup = sub_floder_name.
          incsubdir        = sub_floder_name.
        ELSE.
          CONCATENATE sub_floder_name <wafunction>-functionname INTO incsubdir SEPARATED BY slashseparator.
        ENDIF.

        mainsubdir = sub_floder_name.
        PERFORM directory_create USING lv_flag " 'P' PC ; 'S' SERVER
                                       sub_floder_name.
      ENDIF.
    ...
    " ---added by lijq-----begin-----
    " ---In the same FG, only download one: global-FG_FgName.html file.
    IF gv_merge_flag = 'X'.
      " Download main source code
      PERFORM readfunctionanddownload
        USING iemptytextelements[] " <wafunction>-itextelements[]
              iemptyselectiontexts[] "<wafunction>-iselectiontexts[]
              iemptymessages[] "<wafunction>-imessages[]
              <wafunction>-functionname <wafunction>-functionmaininclude <wafunction>-functiontitle userfilepath
              fileextension mainsubdir converttohtml functiondocumentationexists customernamerange getincludes
              getdictstruct slashseparator savetoserver displayprogressmessage locserverfilesystem addbackground.
      " Download top include
      IF gv_different_flag = 'X'.
        PERFORM readincludeanddownload
          USING <wafunction>-itextelements[] " iemptytextelements[]
                <wafunction>-iselectiontexts[] " iemptyselectiontexts[]
                <wafunction>-imessages[] " iemptymessages[]
                iemptyguititles[] <wafunction>-topincludename gv_functiongroup "<wafunction>-functionname
                <wafunction>-topincludename "<wafunction>-functiontitle
                is_function userfilepath fileextension mainsubdir converttohtml customernamerange getincludes
                getdictstruct slashseparator savetoserver displayprogressmessage locserverfilesystem addbackground.
    ...
    ELSE.
      ...
      "    Download main source code
      PERFORM readfunctionanddownload
        USING <wafunction>-itextelements[] <wafunction>-iselectiontexts[] <wafunction>-imessages[]
              <wafunction>-functionname <wafunction>-functionmaininclude <wafunction>-functiontitle userfilepath
              fileextension mainsubdir converttohtml functiondocumentationexists customernamerange getincludes
              getdictstruct slashseparator savetoserver displayprogressmessage locserverfilesystem addbackground.
      " Download top include
      PERFORM readincludeanddownload
        USING iemptytextelements[] iemptyselectiontexts[] iemptymessages[] iemptyguititles[] <wafunction>-topincludename
              <wafunction>-functionname <wafunction>-functiontitle is_function userfilepath fileextension mainsubdir
              converttohtml customernamerange getincludes getdictstruct slashseparator savetoserver
              displayprogressmessage locserverfilesystem addbackground.
    ENDIF.
    " ---added by lijq-----begin-----
    " ---save  screens & dictionary floder into the FM floder.
    IF mainsubdir IS INITIAL.
      mainsubdir = <wafunction>-functionname.
    ENDIF.
    " ---added by lijq-----end-----

    IF downloaddocumentation IS NOT INITIAL.
      PERFORM downloadfunctiondocs
        USING <wafunction>-functionname
              <wafunction>-functiontitle
              userfilepath
              fileextension
    ...
    ENDIF.
    " Download screens.
    IF <wafunction>-iscreenflow[] IS NOT INITIAL.
      PERFORM downloadscreens
        USING <wafunction>-iscreenflow[]
              <wafunction>-progname
              userfilepath
              textfileextension
              mainsubdir
    ...
    " Download GUI titles
    IF <wafunction>-iguititle[] IS NOT INITIAL.
      PERFORM downloadguititles
        USING <wafunction>-iguititle
              userfilepath
              textfileextension
              mainsubdir
              slashseparator
    ...
    " ---changed by lijq ---begin----
    "     LOOP AT <wafunction>-iincludes ASSIGNING <wainclude>.
    LOOP AT <wafunction>-iincludes ASSIGNING <wainclude> WHERE IncludeName <> <wafunction>-topincludename.
      " ---changed by lijq ---end----
      PERFORM readincludeanddownload
        USING iemptytextelements[] iemptyselectiontexts[] iemptymessages[] iemptyguititles[] <wainclude>-includename
              space <wainclude>-includetitle is_program userfilepath fileextension incsubdir converttohtml
              customernamerange getincludes getdictstruct slashseparator savetoserver displayprogressmessage
              locserverfilesystem addbackground.

    ENDLOOP.

    " Download all dictionary structures
    IF <wafunction>-idictstruct[] IS NOT INITIAL.
      PERFORM downloadddstructures
        USING <wafunction>-idictstruct[]
    ...
    IF <wafunction>-itabletypes[] IS NOT INITIAL.
      PERFORM downloadddtabletypes
        USING <wafunction>-itabletypes[]
    ...
    IF <wafunction>-itransformations[] IS NOT INITIAL.
      PERFORM downloadxslt
        USING <wafunction>-itransformations[]
              userfilepath
              fileextension
              htmlfileextension
              textfileextension
...
" ------------------------------------------------------------------------
FORM readincludeanddownload
  USING iloctextelements              LIKE dumitexttab[]
...
" ------------------------------------------------------------------------
FORM readclassanddownload
  USING walocclass                    TYPE tclass
        VALUE(classname)
        VALUE(functionname)
        VALUE(overideprogtype)
        VALUE(userfilepath)
  ...
  IF walocclass-iconcepts[] IS NOT INITIAL.
...
" ------------------------------------------------------------------------
FORM readmethodanddownload
  USING walocmethod                   TYPE tmethod
        VALUE(methodname)
        VALUE(methodkey)
        VALUE(functionname)
        VALUE(overideprogtype)
...
FORM readfunctionanddownload
  ...
  IF savetoserver IS INITIAL.
    " ---added by lijq ----begin------
    " --upper case FM name
    DATA lv_pos_from TYPE syst_fdpos.
    DATA lv_pos_to   TYPE syst_fdpos.
    DATA lv_st1      TYPE char200.
    CLEAR:lv_pos_from,
           lv_pos_to,
           lv_st1.
    IF is_function = 'F'.
      SEARCH localfilenamewithpath FOR localfilenameonly.
      IF sy-subrc IS INITIAL.
        lv_pos_from = sy-fdpos.
        lv_pos_to = strlen( functionname ).
        lv_st1 = localfilenamewithpath.
        lv_st1+lv_pos_from(lv_pos_to) = functionname.
        CLEAR localfilenamewithpath.
        localfilenamewithpath = lv_st1.
      ENDIF.
    ENDIF.
    " ---added by lijq ----end------
    PERFORM savefiletopc
...
FORM buildfilename
  ...
  IF downloadtoserver IS INITIAL.
    IF frontendopsystem = non_unix.
      IF additionalsubdirectory IS NOT INITIAL.
  ...
  CASE downloadtype.
    ...
    WHEN is_function.
      IF additionalsubdirectory IS INITIAL.
        ...
        ELSE.
          IF includename CS mainfunctionno AND mainfunctionno IS NOT INITIAL.
            CONCATENATE userpath slashseparator objectname
      ...
      ELSE.
        " ----added by lijq----begin-----
        " ---remove FM floder from the path
        IF gv_merge_flag = 'X'.
          FIND 'top' IN includename IGNORING CASE.
          IF sy-subrc = 0.
            CONCATENATE userpath slashseparator additionalsubdirectory
...
*                                   slashseparator objectname
          ...
          newsubdirectory = additionalsubdirectory.
          CONCATENATE userpath slashseparator additionalsubdirectory INTO completepath.
...
" ------------------------------------------------------------------------
FORM savefiletopc
  USING idownload                     TYPE STANDARD TABLE
  ...
  IF displayprogressmessage IS NOT INITIAL.
    CONCATENATE `Downloading: ` filename INTO statusmessage.
    PERFORM displaystatus USING statusmessage
                                0.
  ENDIF.
...
FORM checkpathexists USING VALUE(path)
  ...
  CALL FUNCTION 'PFL_CHECK_DIRECTORY'
    EXPORTING  directory                   = castdirectory
*               WRITE_CHECK                 = ' '
    EXCEPTIONS pfl_dir_not_exist           = 1
...
FORM createserverdirectory USING VALUE(path)
  ...
  CALL FUNCTION 'SXPG_COMMAND_EXECUTE'
    EXPORTING  commandname                   = 'ZDTX_MKDIR'
               ...
               stderr                        = 'X'
               terminationwait               = 'X'
    IMPORTING  status                        = funcstatus
    TABLES     exec_protocol                 = iserveroutput[]
...
FORM downloadfunctiondocs
  ...
  CALL FUNCTION 'FUNCTION_IMPORT_DOKU'
    ...
    TABLES     dokumentation      = idocumentation
  ...
  IF sy-subrc <> 0 OR ( iscriptlines[] IS INITIAL ).
    RETURN.
  ENDIF.
  ...
  IF NOT ( idocumentation[] IS INITIAL ).
    ...
    LOOP AT idocumentation.
      waline = idocumentation-parameter.
      waline+stringlength = idocumentation-stext.
...
" downloadClassDocs...
" ---------------------------------------------------------------------------------------
FORM downloadclassdocs
  USING VALUE(classname)              TYPE seoclsname
        VALUE(userfilepath)
        VALUE(fileextension)
        VALUE(converttohtml)
        VALUE(slashseparator)
...
" downloadScreens...
" ---------------------------------------------------------------------------------------
FORM downloadscreens
  USING ilocscreenflow                LIKE dumiscreen[]
        VALUE(programname)
        VALUE(userfilepath)
        VALUE(textfileextension)
        VALUE(subdir)
  ...
  LOOP AT ilocscreenflow ASSIGNING <wascreen>.
    ...
    CALL FUNCTION 'RS_SCRP_HEADER_RAW_TO_CHAR'
      EXPORTING  header_int  = header
      IMPORTING  header_char = wacharheader
      EXCEPTIONS OTHERS      = 1.
...
" downloadGUITitles..
" ---------------------------------------------------------------------------------------
FORM downloadguititles
  USING ilocguititles                 LIKE dumiguititle[]
        VALUE(userfilepath)
        VALUE(textfileextension)
        VALUE(subdir)
        VALUE(slashseparator)
...
FORM downloadprograms
  ...
  DATA iemptymessages       TYPE STANDARD TABLE OF tmessage.
  DATA iemptyguititles      TYPE STANDARD TABLE OF tguititle.
  DATA locconverttohtml     TYPE c LENGTH 1.
  ...
  LOOP AT ilocprogram ASSIGNING <waprogram>.
    ...
    PERFORM readincludeanddownload
      USING <waprogram>-itextelements[] <waprogram>-iselectiontexts[] <waprogram>-imessages[] <waprogram>-iguititle[]
            <waprogram>-progname space <waprogram>-programtitle is_program userfilepath locfileextension
            <waprogram>-progname locconverttohtml customernamerange getincludes getdictstruct slashseparator
            savetoserver displayprogressmessage locserverfilesystem addbackground.

    " Download screens.
    IF <waprogram>-iscreenflow[] IS NOT INITIAL.
      PERFORM downloadscreens
        USING <waprogram>-iscreenflow[]
              <waprogram>-progname
              userfilepath
              textfileextension
              <waprogram>-progname
    ...
    " Download GUI titles
    IF <waprogram>-iguititle[] IS NOT INITIAL.
      PERFORM downloadguititles
        USING <waprogram>-iguititle
              userfilepath
              textfileextension
              <waprogram>-progname
              slashseparator
    ...
    LOOP AT <waprogram>-iincludes ASSIGNING <wainclude>.
      PERFORM readincludeanddownload
        USING iemptytextelements[] iemptyselectiontexts[] iemptymessages[] iemptyguititles[] <wainclude>-includename
              space <wainclude>-includetitle is_program userfilepath fileextension <waprogram>-progname converttohtml
              customernamerange getincludes getdictstruct slashseparator savetoserver displayprogressmessage
              locserverfilesystem addbackground.
    ...
    " Download all dictionary structures
    IF <waprogram>-idictstruct[] IS NOT INITIAL.
      PERFORM downloadddstructures
        USING <waprogram>-idictstruct[]
              ilocdictionaryfilename[]
              userfilepath
              htmlfileextension
              <waprogram>-progname
    ...
    " Download all Table Types
    IF <waprogram>-itabletypes[] IS NOT INITIAL.
      PERFORM downloadddtabletypes
        USING <waprogram>-itabletypes[]
              iloctabletypefilename[]
              userfilepath
              htmlfileextension
              <waprogram>-progname
    ...
    " Download Transformations
    IF <waprogram>-itransformations[] IS NOT INITIAL.
      PERFORM downloadxslt
        USING <waprogram>-itransformations[]
              userfilepath
              fileextension
              htmlfileextension
              textfileextension
    ...
    IF iprogfunctions[] IS NOT INITIAL.
      PERFORM downloadfunctions
        USING iprogfunctions[] ilocdictionaryfilename[] itabletypefilename[] userfilepath fileextension
              <waprogram>-progname downloaddocumentation converttohtml customernamerange getincludes getdictstruct
              textfileextension htmlfileextension sorttablesasc slashseparator savetoserver displayprogressmessage
              locserverfilesystem addbackground.
      CLEAR iprogfunctions[].
    ENDIF.
  ENDLOOP.
...
FORM downloadclasses
  ...
  " ---added by lijq ---begin----
  DATA lv_downloadfolder TYPE string.
  DATA lv_path           TYPE string.
  DATA lv_flag           TYPE c LENGTH 1.

  FIELD-SYMBOLS <fs_spot> LIKE is_spot.
  FIELD-SYMBOLS <fs_badi> LIKE is_badi.

  lv_downloadfolder = userfilepath.
  " ---added by lijq ---end----
  LOOP AT ilocclasses ASSIGNING <waclass>.

    " ---added by lijq ---begin----
    IF rspot = 'X' OR rbadi = 'X'.
      CLEAR:is_spot,
             is_badi,
             lv_path.
      IF rspot = 'X'.
        READ TABLE it_spot INTO is_spot WITH KEY class_name = <waclass>-clsname.
        IF sy-subrc = 0.
          IF is_spot-path IS INITIAL.
            CONCATENATE 'ENH_SPOT---' is_spot-spotname INTO lv_path.
            lv_path = condense( val  = lv_path
                                from = ` `
                                to   = `` ).
            LOOP AT it_spot ASSIGNING <fs_spot> WHERE spotname = is_spot-spotname.
              <fs_spot>-path = lv_path.
            ENDLOOP.

            CONCATENATE userfilepath '\' lv_path INTO lv_downloadfolder.
            lv_downloadfolder = condense( val  = lv_downloadfolder
                                          from = ` `
                                          to   = `` ).
          ELSE.
            CONCATENATE userfilepath '\' is_spot-path INTO lv_downloadfolder.
            lv_downloadfolder = condense( val  = lv_downloadfolder
                                          from = ` `
                                          to   = `` ).
          ENDIF.
        ENDIF.
      ELSE. " rbadi = 'X'.
        READ TABLE it_badi INTO is_badi WITH KEY imp_class = <waclass>-clsname.
        IF sy-subrc = 0.
          IF is_badi-path IS INITIAL.
            CONCATENATE 'BADI---' is_badi-exit_name INTO lv_path.
            lv_path = condense( val  = lv_path
                                from = ` `
                                to   = `` ).
            LOOP AT it_badi ASSIGNING <fs_badi> WHERE exit_name = is_badi-exit_name.
              <fs_badi>-path = lv_path.
            ENDLOOP.
            ...
            lv_downloadfolder = condense( val  = lv_downloadfolder
                                          ...
                                          to   = `` ).
          ELSE.
            CONCATENATE userfilepath '\' is_badi-path INTO lv_downloadfolder.
            lv_downloadfolder = condense( val  = lv_downloadfolder
                                          from = ` `
                                          to   = `` ).
          ENDIF.
    ...
    " Download the class
    PERFORM readclassanddownload
      USING <waclass> <waclass>-clsname space is_class lv_downloadfolder " userfilepath  "changed by lijq
            fileextension space converttohtml customernamerange getincludes getdictstruct slashseparator savetoserver
            displayprogressmessage locserverfilesystem addbackground.

    " Download all of the methods
    LOOP AT <waclass>-imethods ASSIGNING <wamethod>.
      additionalsubdirectory = <waclass>-clsname.
      PERFORM readmethodanddownload
        USING <wamethod> <wamethod>-cmpname <wamethod>-methodkey space is_method lv_downloadfolder " userfilepath
              fileextension additionalsubdirectory converttohtml customernamerange getincludes getdictstruct
              slashseparator savetoserver displayprogressmessage locserverfilesystem addbackground.

    ENDLOOP.

    " Download all dictionary structures
    IF <waclass>-idictstruct[] IS NOT INITIAL.
      PERFORM downloadddstructures
        USING <waclass>-idictstruct[]
              ilocdictionaryfilename[]
              lv_downloadfolder " userfilepath
              htmlfileextension
              <waclass>-clsname
    ...
    " Download all Table Types
    IF <waclass>-itabletypes[] IS NOT INITIAL.
      PERFORM downloadddtabletypes
        USING <waclass>-itabletypes[]
              iloctabletypefilename[]
              lv_downloadfolder " userfilepath
              htmlfileextension
              <waclass>-clsname
    ...
    " Download Transformations
    IF <waclass>-itransformations[] IS NOT INITIAL.
      PERFORM downloadxslt
        USING <waclass>-itransformations[]
              lv_downloadfolder " userfilepath
              fileextension
              htmlfileextension
              textfileextension
    ...
    IF iclassfunctions[] IS NOT INITIAL.
      PERFORM downloadfunctions
        USING iclassfunctions[] ilocdictionaryfilename[] iloctabletypefilename[] lv_downloadfolder " userfilepath
              fileextension <waclass>-clsname downloaddocumentation converttohtml customernamerange getincludes
              getdictstruct textfileextension htmlfileextension sorttablesasc slashseparator savetoserver
              displayprogressmessage locserverfilesystem addbackground.
      CLEAR iclassfunctions[].
    ENDIF.

    IF downloaddocumentation = true.
      PERFORM downloadclassdocs
        USING <waclass>-clsname
              lv_downloadfolder " userfilepath
              fileextension
              converttohtml
...
" ------------------------------------------------------------------------
FORM downloadxslt
  USING iloctransformation            LIKE itransformations[]
        VALUE(userfilepath)
        VALUE(fileextension)
        VALUE(htmlfileextension)
        VALUE(textfileextension)
...
FORM convertddtohtml
  USING ilocdictstructure    LIKE dumidictstructure[]
        ilochtml             LIKE dumihtml[]
        VALUE(tablename)
        VALUE(tabletitle)
        VALUE(sorttablesasc)
  ...
  DATA rowcounter     TYPE c LENGTH 3.
  " The base type of the field we are reading
  DATA wfieldbasetype TYPE c LENGTH 1.
  ...
  IF sorttablesasc IS NOT INITIAL.
...
" ---------------------------------------------------------------------------------------
" addDomaintoHTML...   Add domain values into the HTML
" ---------------------------------------------------------------------------------------
...
" ---------------------------------------------------------------------------------------
FORM convertcodetohtml
  USING icontents                          LIKE dumihtml[]
        VALUE(programname)
  ...
  DATA head              TYPE c LENGTH 255.
  DATA tail              TYPE c LENGTH 255.
  DATA mytabix           TYPE sytabix.
  ...
  IF gv_merge_flag = 'X'. " added by lijq
    CONCATENATE `     <td><h2>Code listing for: ` gv_functiongroup `</h2>` INTO htmltable.
  ELSE.
      ...
      IF wacontent+0(1) <> asterix.
        ...
        IF sourcecodetype <> is_documentation.
          ...
          IF ( wacontent CS callfunction ) AND ( wacontent <> 'DESTINATION' ).
            ...
            wanextline = to_upper( wanextline ).
        ...
        IF wacontent CS include OR wacontent CS lowinclude.
          ...
          TRY.
              IF     ( tail+0(1) = 'Y' OR tail+0(1) = 'Z' OR tail+0(1) = 'y' OR tail+0(1) = 'z' OR tail CS customernamerange OR tail+0(2) = 'mz' OR tail+0(2) = 'MZ' )
                 AND getincludes IS NOT INITIAL AND tail NS structure AND tail NS lowstructure.
                ...
                wacontent = copyofcurrentline.
              ELSE.
                IF getdictstructures IS NOT INITIAL.
        ...
        IF sy-tabix > 1.
          ...
          ELSE.
            IF foundasterix = true.
              ...
              ENDIF.
              IF gv_merge_flag = 'X'. " added by lijq -
                CONCATENATE copyofcurrentline 'global-' gv_functiongroup  " functionName
        ...
        IF wacontent CS include OR wacontent CS lowinclude.
          ...
          TRY.
              IF (    tail+0(1)  = 'Y'               OR tail+0(1) = 'Z'  OR tail+0(1) = 'y' OR tail+0(1) = 'z'
                   OR tail      CS customernamerange OR tail+0(2) = 'mz' OR tail+0(2) = 'MZ' ) AND getincludes IS NOT INITIAL.
...
" ---------------------------------------------------------------------------------------
FORM addhtmlheader
  USING ilocheader            LIKE dumihtml[]
        VALUE(title)
        VALUE(addbackground)  TYPE abap_bool
...
FORM addgenericstyles USING ilocheader           LIKE dumihtml[]
  ...
  IF addbackground IS NOT INITIAL.
...
FORM buildfootermessage USING returnmessage.
  CONCATENATE `Extracted by Mass Download version `
              ...
              sy-datum+0(4) `. Sap Release ` sy-saprl
              ' [ Updated to v1.5.5.4 by lijq21@qq.com]' INTO returnmessage.
ENDFORM.
...
FORM filltreenodetables USING ilocdictionary  LIKE idictionary[]
  ...
  DATA runtimechar      TYPE c LENGTH 10.
...
FORM displaytree USING iloctreedisplay LIKE itreedisplay[].
  ...
  CALL FUNCTION 'RS_TREE_CONSTRUCT'
    TABLES     nodetab            = itreedisplay
    EXCEPTIONS tree_failure       = 1
               id_not_found       = 2
  ...
  CALL FUNCTION 'RS_TREE_EXPAND'
    EXPORTING  node_id   = watreedisplay-id
               all       = 'X'
...
*&---------------------------------------------------------------------*
FORM directory_create USING pv_flag
                            p_name.
  ...
  IF pv_flag = 'P'. " PC
    ...
    CONCATENATE lv_path '\' p_name INTO p_name.
    CONDENSE p_name NO-GAPS.
    ASSIGN p_name TO <new_dir>.

    cl_gui_frontend_services=>directory_create( EXPORTING  directory               = <new_dir>
                                                CHANGING   rc                      = rc
